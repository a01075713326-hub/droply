...
async function cryptoRank() {
  ...
  return {
    ...
    console.log(
      `CryptoRank: enriching ${projects.length} projects`
    );
    ...
    for (let i = 0; i < projects.length; i++) {
      ...
      try {
        ...
        // Prevent activity labels from becoming blockchain names.
        if (
          result.chain &&
          /^(mainnet|testnet|trading|mainnet trading)$/i.test(
            result.chain.trim()
          )
        ) {
          result.chain = project.chain || "Multiple";
        }

        enriched.push(result);

        console.log(
          `CryptoRank: enriched ${i + 1}/${projects.length} — ${project.name}`
        );
      } catch (error) {
        console.warn(
          `CryptoRank: enrichment failed for ${project.name}: ${
            error?.message || "unknown error"
          }`
        );
    ...
    console.log(
      `CryptoRank: finished enrichment (${enriched.length} projects)`
    );
